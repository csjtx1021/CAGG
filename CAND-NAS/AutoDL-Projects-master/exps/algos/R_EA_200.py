...
def train_and_eval(arch, nas_bench, extra_info, dataname='cifar10-valid', use_012_epoch_training=True,rand_seed=None):
  
  use_012_epoch_training = False
  ...
  if use_012_epoch_training and nas_bench is not None:
    ...
    index = nas_bench.query_index_by_arch( arch )
    
    results = nas_bench.query_by_index(index, dataname) # a dict of all trials, where the key is the seed
    print ('>>>>>>>> there are {:} trials for this architecture on {:}'.format(len(results), dataname))
          
    valid_error_list = [results[trial_seed_i].get_eval("x-valid")["accuracy"] for trial_seed_i in list(results.keys())]
              
    #randomly choose one from multipy trials
    res = random.choice(valid_error_list)
              
    time_cost = 0.
                  
    if dataname == "cifar10-valid":
        results = nas_bench.query_by_index(index, "cifar10") # a dict of all trials, where the key is the seed
    ...
    print(">>>>>>>> get result in NASBench201, valid_acc = %s (use in search), cost = %s, and acc_info = %s (not use in search, just test)"%(res,time_cost,acc_info))
    ...
    import csv,time
    #filename = "output/search-cell-nas-bench-201/%s-200-410eval-%s-%s.csv"%(method_name,dataname,args.rand_seed)
    filename = "output/search-cell-nas-bench-201/%s-200-800eval-%s-%s.csv"%(method_name,dataname,rand_seed)
    with open(filename,"a",newline="") as datacsv:
        csvwriter = csv.writer(datacsv)
        csvwriter.writerow([valid_acc,arch,index,"%s"%acc_info])
  ...
  #time_cost = 23.998. #500 evaluations
  #time_cost = 11.999 #1000 evaluations
  #time_cost = 29.25 #410 evaluations
  time_cost = 14.99 #800 evaluations
...
def regularized_evolution(cycles, population_size, sample_size, time_budget, random_arch, mutate_arch, nas_bench, extra_info, dataname):
  ...
  while len(population) < population_size:
    ...
    model.accuracy, time_cost = train_and_eval(model.arch, nas_bench, extra_info, dataname, rand_seed=args.rand_seed)
  ...
  while total_time_cost < time_budget:
    ...
    child.accuracy, time_cost = train_and_eval(child.arch, nas_bench, extra_info, dataname,rand_seed=args.rand_seed)
